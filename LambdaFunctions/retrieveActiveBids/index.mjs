      ...
      pool.query("SELECT username FROM buyerTable WHERE username=? and authKey=?", [buyerUsername, authKey], (error, rows) => {
          if (error) { return reject(error) }
              if ((rows) && (rows.length != 0)) {
                pool.query("SELECT itemName, bidAmount, date FROM itemTable JOIN bidTable ON itemTable.itemID = bidTable.itemId WHERE bidTable.buyerUsername=? and active=True", [buyerUsername], (error, rows) => {
                    if (error) { return reject(error) }
                    if ((rows) && (rows.length != 0)) {
                      return resolve(rows)
                    } else {
