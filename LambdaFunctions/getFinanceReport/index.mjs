...
export const handler = async (event) => {
  ...
  let getSellerFunds = () => {
    return new Promise((resolve, reject) => {
        pool.query("select SUM(revenue) as sum from sellerTable", [], (error, rows) => {
            if (error) { return reject(error) }
            if ((rows) && (rows.length != 0)) {
                return resolve(rows)
            } else {
  ...
  let getPItem = () => {
    return new Promise((resolve, reject) => {
        pool.query("select * from (select itemID, count(itemID) as numItems from bidTable group by itemID) as AAT where numItems in (select MAX(numItems) from (select itemID, count(itemID) as numItems from bidTable group by itemID) as dt) group by itemID;", [], (error, rows) => {
            if (error) { return reject(error) }
            if ((rows) && (rows.length != 0)) {
                return resolve(rows)
            } else {
