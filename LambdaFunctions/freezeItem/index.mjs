...
export const handler = async (event) => {
  ...
  let freezeItem = (itemId) => {
    return new Promise((resolve, reject) => {
        pool.query("update itemTable set frozen=1 where itemId = ?", [itemId], (error, results) => {
            console.log(results)
  ...
  if(result){
    let froze = await freezeItem(event.itemId)
    if(froze){
      response ={
        statusCode: 200,
        body: "Item Has been frozen"
      }
    }else{
        response ={
                statusCode: 400,
                body: "Item Has not Been frozen"
              }      
    }
